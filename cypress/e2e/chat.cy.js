...
describe('customer support chat', () => {
  ...
  it('displays the user message on input + enter and displays the bots response ', () => {
    ...
    cy.get(`[data-test="chat-input"]`).should('be.visible').type(`${message}{enter}`)
    cy.get(`[data-test="chat-bubble"]:first-of-type [data-test="chat-bubble-text"]`).should(
      'contain.text',
      message
    )

    // Test bot response
    cy.get(`[data-test="chat-bubble"]:last-of-type [data-test="chat-bubble-text"]`).should(
      'contain.text',
